...
# I can add letter and for C - V - R they are a b b
...
m5<-lm(specrich ~ graztreat+block+graztreat:block, data=dat) # same
m5<-lm(specrich ~ graztreat*block, data=dat)
anova(m5)
...
anova(m8)
AIC(m8,m7)
# yes it matters, the difference between the treatments depends on the block
...
# first fit a model with only block
m9<-lme4::glmer(specrich ~ (1|block), data=dat, family=poisson(log))

# add treatment to the model, assuming fixed slopes (i.e. treatment effect is the same for every block)
m10<-lme4::glmer(specrich ~ graztreat + (1|block), data=dat, family=poisson(log))
anova(m10,m9)
...
m11<-lme4::glmer(specrich ~ graztreat + (graztreat|block), data=dat, family=poisson(log))
anova(m11,m10)
