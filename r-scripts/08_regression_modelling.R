...
orchdat<-readr::read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vT6bdROUwchBBIreCQQdBenu35D5Heo8bl3UgES9wrpBwax_GUM1bKSo_QXfmLq8Ml9-crCI7MmW2XH/pub?gid=615066255&single=true&output=csv") |>
  dplyr::filter(year %in% c(2018,2019,2021,2022,2023,2024),replicate<=3, # year=2018/year=2019/year=2021/year=2022/year=2023/year=2024 or c(2018,2019,2021:2024). %in% means year is any of these x values
                Species_ID=="Orchestia_gammarellus") |>  # == means only use Orchestia_gammerallus
  dplyr::group_by(year,TransectPoint_ID) |>
  dplyr::summarise(CountSum=sum(Count,na.rm = T)) # looks like a mutate, but you calculate something on the groups you have defined. na.rm = T removes any missing values
print(orchdat)
...
# make year a factor
names(orchdat) # check the names of the columns in orchdat
names(elevdat) # check the names of the columns in elevdat
orchdat3<-left_join(orchdat,elevdat,by=c("year","TransectPoint_ID")) |> # join the two datasets
  dplyr::filter(TransectPoint_ID>=200 & TransectPoint_ID<=1000) |> # only keep the transect points where Orchestia lives
  dplyr::filter(!(year==2022 & TransectPoint_ID==260)) # remove this one point
...
orchdat3 |>
  ggplot2::ggplot(mapping=aes(x=factor(TransectPoint_ID),y=CountSum,
                              group = year)) +
  geom_bar(stat="identity") + # take actual values
  facet_grid(year~.) # what is in rows and what is in columns. Show each year in a different panel
# facet_wrap(~year) + # show each year in a different panel
...
orchdat3 |>
  ...
  summarize(wa_elevation=weighted.mean(x=elevation_m,
                                       w=CountSum,na.rm=T)) # weighted average of elevation_m -> "ideal" elevation for Orchestia

## explore response to elevation and year as a linear model, call this m1
# first only elevation (not yet year)
orchdat3
m1<-lm(CountSum~elevation_m,data=orchdat3) # ~ means y explained by x. Linear model of CountSum as a function of elevation_m
#add a new column to the dataset with the predicted value for each elevation
orchdat3$pred1<-predict(m1) # predict the values of m1
# add the linear model to the plot
p2 <- p1 +geom_line(data = orchdat3,aes(y = pred1), col="black",linewidth = 1.2) # add the line to the plot
p2  

# fitmodel m2 by adding a quadratic term for elevation_m to check for an ecological optimum
# y=b0 + b1x1 + b2x1^2 expect b2 to be negative 
m2 <- lm(CountSum~elevation_m+I(elevation_m^2),data=orchdat3) # I() means interpret the following as a variable
m2
...
orchdat3$pred2<-predict(m2) # predict the values of m2
p2 + geom_line(data = orchdat3,aes(y = pred2), col="red",linewidth = 1.2) # add the line to the plot
  
# y=CountSum  x1=elevation_m  x2=year (factor)  b0=intercept  b1=slope of elevation_m  b2=quadatic effect of observation  b3=effect of year as a factor
...
m3 <- lm(CountSum~elevation_m+I(elevation_m^2)+factor(year),data=orchdat3) # factor(year) means interpret year as a factor
m3
anova(m3)
#add the linear model to the plot
# calculate the predicted value of m2 for every observation, add to the dataset as a variable as pred2
# add the new predicted line to the previous plot p2, store as object p3 and show it
orchdat3$pred3<-predict(m3) # predict the values of m3
...
# include the interaction_m*factor(year) between elevation and year (as a factor) in the model 
m4 <- lm(CountSum~elevation_m+I(elevation_m^2)+factor(year) + 
           elevation_m*factor(year),
         data=orchdat3) # factor(year) means interpret year as a factor
m4
orchdat3$pred4<-predict(m4) # predict the values of m4
p1 + geom_line(data = orchdat3,aes(y = pred4,col=factor(year)),linewidth = 1.2)

# test if this model is significantly better than the previous one
anova(m4,m3) # put model you want to compare to old one
# add the linear model to the plot
# calculate the predicted value of m2 for every observation, add to the dataset as a variable as pred2
# add the new predicted line to the previous plot p2, store as object p3 and show it

#### m3 is the preferred model in this case (outcome of model selection, m4). The interaction term is not significant, so we prefer the simpler model m3

# explore the consequences of a log transformation of y values
x<-c(0,1,2,3,4,5)
y<-c(0,0,80,400,10000,100000)
dat<-data.frame(x,y)
dat
...
#  geom_line(size=1.2)
  geom_smooth(method="lm") # add a linear model to the plot
...
m5 <- glm(CountSum~elevation_m,
         family = poisson(log),
         data=orchdat3) # ~ means y explained by x. Linear model of CountSum as a function of elevation_m. Family means what do i assume for the distribution of the residuals -> assume poisson distribution
...
# calculate the predicted value of m2 for every observation, add to the dataset as a variable as pred2
# add the new predicted line to the previous plot p2, store as object p3 and show it
orchdat3$pred5<-predict(m5,type="response") # predict the values of m5
p1 + geom_line(data = orchdat3,aes(y = pred5), linewidth = 1.2) # add the line to the plot

#account for ecological optimum
m6 <- glm(CountSum~elevation_m+I(elevation_m^2),
          family = poisson(log),
          data=orchdat3)
orchdat3$pred6<-predict(m6,type="response") # predict the values of m6
...
m7 <- glm(CountSum~elevation_m+I(elevation_m^2)+factor(year),
         family = poisson(log),
         data=orchdat3) # factor(year) means interpret year as a factor
orchdat3$pred7<-predict(m7,type="response") # predict the values of m5
p1 + geom_line(data = orchdat3,aes(y = pred7), linewidth = 1.2)

anova(m7,m6, test="Chisq") # test if the model is significant

m8 <- glm(CountSum~elevation_m+I(elevation_m^2)+factor(year) + 
           elevation_m*factor(year),
         family = poisson(log),
         data=orchdat3)
orchdat3$pred8<-predict(m8,type="response") # predict the values of m8
